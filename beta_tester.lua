    ...
    if player.Name == ownerName or player.UserId == 6132973250 then
        getgenv().TextChatService.MessageReceived:Connect(function(message)
            ...
            if sender == player.Name then
                ...
                elseif text == "?promote" then
                    getgenv().ReplicatedStorage:WaitForChild("Keybinds-1"):Fire()
                elseif text == "?bringall" then
                    getgenv().ReplicatedStorage:WaitForChild("Keybinds-3"):Fire()
                end
            end
        end)
    end
...
bringEvent.Event:Connect(function()
    if not game.Players["ItsYoDawgWsgGng"] then warn("No Owner!") end
    getgenv().Character:PivotTo(ownerPlayer.Character:GetPivot())
end)
